...
# access test webpage
with open(r'C:\Python Programs\Web Monitor\NEWPAGE.txt', 'r') as file:
    wp_new = file.read()

# open subscriber text file
...
# separate each email and corresponding list of locations. locations will be used to filter content for each email
for i in subs:
    temp = i.split(',')
    sub_emails.append(temp[0])
    sub_groups.append(temp[1:])
